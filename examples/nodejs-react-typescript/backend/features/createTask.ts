...
export const createTask = defineFeature<CreateTaskInput, { task: Task }>({
  name: "create-task",
  description: "Create one task with a non-empty title and optional priority.",
  ...
  async execute(context) {
    ...
    const task: Task = {
      ...
      title,
      priority: normalizePriority(context.input.priority),
      completed: false,
      createdAt: await context.capabilities["clock.now"](),
      completedAt: null,
    };
