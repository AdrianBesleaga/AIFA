...
export function createFileTaskStore(filePath: string): TaskStore {
  async function readTasks(): Promise<Task[]> {
    try {
      return JSON.parse(await readFile(filePath, "utf8")) as Task[];
    } catch (error: unknown) {
  ...
  return {
    ...
    async load(taskId) {
      return (await readTasks()).find((task) => task.id === taskId) ?? null;
    },
    async save(task) {
      const tasks = await readTasks();
      await writeTasks(tasks.map((stored) => (stored.id === task.id ? task : stored)));
      return task;
    },
    async delete(taskId) {
      const tasks = await readTasks();
    ...
    async list() {
      return (await readTasks()).sort((left, right) =>
