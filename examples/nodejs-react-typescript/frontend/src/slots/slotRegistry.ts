...
export interface SlotContribution<Name extends SlotName> {
  ...
  render: (properties: {
    ...
    contribution: SlotContribution<Name>;
  }) => ReactNode;
}

const contributions = new Map<SlotName, unknown[]>();

export function registerSlotContribution<Name extends SlotName>(
  contribution: SlotContribution<Name>,
): void {
  ...
  const slotContributions = (contributions.get(contribution.slot) ??
    []) as SlotContribution<Name>[];
