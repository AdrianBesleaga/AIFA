import assert from "node:assert/strict";
import test from "node:test";

import { completeTask } from "../backend/features/completeTask.ts";
import { createTask } from "../backend/features/createTask.ts";
import { deleteTask } from "../backend/features/deleteTask.ts";
import { listTasks } from "../backend/features/listTasks.ts";
import { reopenTask } from "../backend/features/reopenTask.ts";
import { createAifaRuntime } from "../backend/runtime/aifaRuntime.ts";
import { createMemoryTaskStore } from "../backend/storage/memoryTaskStore.ts";

const actor = {
  id: "test-user",
};

function createRuntime(seed = []) {
  let nextId = 1;

  return createAifaRuntime({
    taskStore: createMemoryTaskStore(seed),
    idFactory: {
      next() {
        const id = `task-${nextId}`;
        nextId += 1;
        return id;
      },
    },
    clock: () => new Date("2026-08-14T10:00:00.000Z"),
  });
}

test("creates an active task with priority", async () => {
  const runtime = createRuntime();

  const result = await runtime.run(
    createTask,
    { title: "  Build slot UI  ", priority: "high" },
    actor,
  );

  assert.equal(result.ok, true);
  assert.equal(result.value.task.id, "task-1");
  assert.equal(result.value.task.title, "Build slot UI");
  assert.equal(result.value.task.priority, "high");
  assert.equal(result.value.task.completed, false);
  assert.equal(runtime.auditLog[0].type, "task.created");
});

test("defaults unknown priority to medium", async () => {
  const runtime = createRuntime();

  const result = await runtime.run(
    createTask,
    { title: "Review contract", priority: "urgent" },
    actor,
  );

  assert.equal(result.value.task.priority, "medium");
});

test("rejects empty task titles", async () => {
  const runtime = createRuntime();

  const result = await runtime.run(createTask, { title: "   " }, actor);

  assert.equal(result.ok, false);
  assert.equal(result.error.code, "invalid_title");
});

test("lists tasks in creation order", async () => {
  const runtime = createRuntime([
    createSeedTask("task-2", "Second", "medium", "2026-08-14T10:02:00.000Z"),
    createSeedTask("task-1", "First", "low", "2026-08-14T10:01:00.000Z"),
  ]);

  const result = await runtime.run(listTasks, {}, actor);

  assert.deepEqual(
    result.value.tasks.map((task) => task.id),
    ["task-1", "task-2"],
  );
});

test("completes, reopens, and deletes a task", async () => {
  const runtime = createRuntime([createSeedTask("task-1", "Ship example")]);

  const completeResult = await runtime.run(completeTask, { taskId: "task-1" }, actor);
  const reopenResult = await runtime.run(reopenTask, { taskId: "task-1" }, actor);
  const deleteResult = await runtime.run(deleteTask, { taskId: "task-1" }, actor);
  const listResult = await runtime.run(listTasks, {}, actor);

  assert.equal(completeResult.value.task.completed, true);
  assert.equal(reopenResult.value.task.completed, false);
  assert.equal(deleteResult.value.deleted, true);
  assert.equal(listResult.value.tasks.length, 0);
  assert.deepEqual(
    runtime.auditLog.map((entry) => entry.type),
    ["task.completed", "task.reopened", "task.deleted"],
  );
});

test("returns not_found for missing task actions", async () => {
  const runtime = createRuntime();

  const completeResult = await runtime.run(completeTask, { taskId: "missing" }, actor);
  const reopenResult = await runtime.run(reopenTask, { taskId: "missing" }, actor);
  const deleteResult = await runtime.run(deleteTask, { taskId: "missing" }, actor);

  assert.equal(completeResult.error.code, "not_found");
  assert.equal(reopenResult.error.code, "not_found");
  assert.equal(deleteResult.error.code, "not_found");
});

function createSeedTask(id, title, priority = "medium", createdAt = "2026-08-14T10:00:00.000Z") {
  return {
    id,
    title,
    priority,
    completed: false,
    createdAt,
    completedAt: null,
  };
}
